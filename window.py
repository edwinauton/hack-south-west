...
class Window(QWidget):
	...
	def init(self):
		...
		# Titles
		value = format(round(sum(stock.equity for stock in stocks), 2))
		...
		value = format(round(sum(stock.daily_return for stock in stocks), 2))
		...
		self.plot_graph((stocks[0].create_graph()[0], sum(stock.create_graph()[1] for stock in stocks)))
	...
	def update_stocks(self, value, stock):
		...
		with open("stock_record.json") as f:
			data = json.load(f)
			data[stock.ticker] = stock.number_owned
